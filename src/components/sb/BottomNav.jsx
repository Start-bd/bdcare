        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#e0e8e4] z-50 bottom-nav md:hidden">
            <div className="flex items-center justify-around py-2">
                {navItems.map(item => {
                    ...
                    return (
                        <Link key={item.page} to={href} className="flex flex-col items-center gap-0.5 min-w-0 px-2 py-1 group">
                            <FloatingIcon
                                Icon={item.icon}
                                color={active ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-400'}
                                size="sm"
                                float={false}
                            />
